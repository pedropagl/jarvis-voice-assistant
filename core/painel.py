...
_MAX_HISTORICO = 8
_MAX_HISTORICO_PRODUCAO = 60  # ~1h de tendencia (refresh_dados roda a cada 60s)
_STATUS_IMPRIMINDO = {"printing", "imprimindo"}
...
def _escrever(estado: dict) -> None:
    ...
    Se o replace falhar (ex.: arquivo bloqueado por outro processo no
    Windows), o arquivo temporario e removido em vez de ficar largado em
    data/ para sempre — antes disso, uma falha aqui deixava um .tmp orfao
    a cada tentativa malsucedida.
    """
    tmp = None
    try:
...
# ===========================================================================
# Helpers: dados REAIS do catos quando disponível, senão MOCK
# ===========================================================================

def _parse_lista_json(texto: str) -> list[dict]:
    """
    As ferramentas de LISTA do catos devolvem varios objetos JSON concatenados
    ...
    while i < n:
        ...
        if isinstance(obj, dict):
            envolvido = obj.get("result")
            if isinstance(envolvido, list) and len(obj) == 1:
                # Algumas ferramentas do catos (ex.: listar_tickets_ativos) embrulham
                # a lista real dentro de {"result": [...]} em vez de devolver os
                # registros soltos. Sem isso, esse objeto-embrulho era tratado como
                # se fosse UM registro (todos os campos saiam "?" no painel).
                objetos.extend(x for x in envolvido if isinstance(x, dict))
            else:
                objetos.append(obj)
        i = fim
    return objetos
...
def _maquina_de_dados(dados: dict, codigo_fallback: str = "?") -> dict:
    """Converte um objeto de impressora do catos no formato do painel."""
    extrusores_raw = []
...
def _overview_maquinas_todas() -> list[dict]:
    """
    Lista TODAS as impressoras (para o radar do painel mostrar todas de uma vez).
    Tenta dados REAIS do catos primeiro; se indisponivel, cai no mock local.
    """
    if mcp_client.disponivel()[0]:
        try:
            lista = _parse_lista_json(mcp_client.chamar_ferramenta("listar_impressoras", {}))
            if lista:
                return [_maquina_de_dados(m, m.get("name", "?")) for m in lista]
        except Exception as erro:  # noqa: BLE001
            log.debug("Falha ao listar todas as maquinas (catos): %s", erro)
    return [
        {"id": m.get("id"), "model": m.get("model"), "status": m.get("status"),
         "extruders": m.get("extruders", [])}
...
def _demanda_pendente_topo() -> dict | None:
    ...
    (listar_demandas_pendentes do catos). Mostra o que vale a pena produzir agora.
    """
    if not mcp_client.disponivel()[0]:
        return None
    try:
...
def _faltantes_catos(ticket_id: str | None) -> list[dict] | None:
    """
    Pecas faltantes (producao pendente) a partir do catos. Se vier um ticket_id,
    filtra so as demandas daquele ticket; senao, mostra todas as pendentes.
    Devolve None se o catos nao estiver disponivel.
    """
    if not mcp_client.disponivel()[0]:
        return None
    try:
        ...
        log.debug("Falha ao puxar faltantes do catos: %s", erro)
        return None


def _alertas_catos() -> list[str] | None:
    """
    Alertas a partir do estado REAL das impressoras (catos): offline ou em
    estado problematico (erro/manutencao/pausada/abortada). Devolve None se o
    catos nao estiver disponivel (ai o chamador usa o fallback mock).
    """
    if not mcp_client.disponivel()[0]:
        return None
    try:
        ...
        log.debug("Falha ao montar alertas do catos: %s", erro)
        return None


def _info_maquina_catos(codigo: str) -> dict | None:
    """Tenta obter info de máquina do catos (dados reais). Se falhar, None."""
    ...
    try:
        ...
        # Estrutura do catos: material_active.extruders[].{index, nozzle, ...}
        extrusores_raw = []
        mat_ativo = dados.get("material_active", {})
        if isinstance(mat_ativo, dict):
        ...
        return {
            ...
            "extruders": [
                {
                    "id": e.get("index", i),
                    # catos manda 'label'/'name' amigaveis (ex.: "Tough PLA") e
        ...
        log.debug("Falha ao puxar máquina do catos: %s", erro)
        return None


def _info_ticket_catos(ticket_id: str) -> dict | None:
    """Tenta obter info de ticket do catos. Se falhar, None."""
    ...
    try:
        ...
        dados = json.loads(resultado)
        # catos usa: ticket_codigo, requisitante_nome, data_requisicao, emergencia.
        ...
        log.debug("Falha ao puxar ticket do catos: %s", erro)
        return None
...
def _info_maquina(codigo) -> dict | None:
    if not codigo:
        return None
    # Tenta dados REAIS (catos) primeiro
    real = _info_maquina_catos(codigo)
    if real:
        return real
    # Fallback: mock (JSONs)
...
def _info_ticket_e_faltantes(entidades) -> tuple[dict | None, list]:
    ...
    # Tenta dados REAIS (catos) primeiro, se houver ticket_id especifico
    if ticket_id:
        t = _info_ticket_catos(ticket_id)
        if t is None:
            # Fallback mock
...
def _registrar_historico_producao(estado: dict, lista_maquinas: list[dict]) -> None:
    """Guarda uma amostra de 'quantas maquinas imprimindo agora' pra dar tendencia
    ao painel (sparkline), sem precisar de banco de dados a mais.
    """
    total = len(lista_maquinas)
    imprimindo = sum(
        1 for m in lista_maquinas
        if str(m.get("status", "")).lower() in _STATUS_IMPRIMINDO
    )
    historico = estado.get("production_history") or []
    historico.append({
        "t": datetime.now().isoformat(timespec="seconds"),
        "imprimindo": imprimindo,
        "total": total,
    })
    estado["production_history"] = historico[-_MAX_HISTORICO_PRODUCAO:]


def refresh_dados() -> None:
    """
    Atualiza SO os paineis de dados ao vivo (maquina, ticket, faltantes, alertas,
    producao pendente) a partir do catos, PRESERVANDO a conversa: resposta, operador,
    ...
    lista_maquinas = _overview_maquinas_todas()
    estado["machines"] = lista_maquinas
    _registrar_historico_producao(estado, lista_maquinas)
    ...
    faltantes = _faltantes_catos(None)
    if faltantes is not None:
        estado["missing_parts"] = faltantes
    alertas = _alertas_catos()
    if alertas is not None:
        alertas_antigos = set(estado.get("alerts") or [])
        _notificar_alertas_novos(alertas, alertas_antigos)
        estado["alerts"] = alertas

    _escrever(estado)


def _notificar_alertas_novos(alertas_atuais: list[str], alertas_antigos: set[str]) -> None:
    """Manda WhatsApp e/ou notificacao push SO para os alertas que ACABARAM
    de aparecer (nao reenvia a cada refresh_dados os que ja estavam la,
    senao viraria spam). Cada canal e independente: funciona com so um dos
    dois configurado, os dois, ou nenhum (nesse caso nao faz nada).
    """
    from core import push, whatsapp  # import tardio: evita custo se nao forem usados

    usa_whatsapp = whatsapp.disponivel()
    usa_push = push.disponivel()
    if not usa_whatsapp and not usa_push:
        return

    novos = [a for a in alertas_atuais if a not in alertas_antigos]
    for alerta in novos:
        if usa_whatsapp:
            whatsapp.enviar_alerta(alerta)
        if usa_push:
            push.enviar_notificacao("⚠ Alerta da JARVIS", alerta)
...
def atualizar(pergunta: str, resultado: dict,
    ...
    # senao, cai pra uma VISAO GERAL ao vivo (catos) pra TV nunca ficar vazia.
    ...
    # catos — senao a TV fica vazia quando o ticket exibido nao tem demandas.
    ticket_query = entidades.get("ticket")
    faltantes_catos = _faltantes_catos(ticket_query or None)
    if faltantes_catos is not None:
        faltantes = faltantes_catos
    ...
    novo = {
        ...
        "machine": maquina,
        "machines": _overview_maquinas_todas(),
