...
_VERBOS_ACAO_LIBERADA = re.compile(
    ...
    r"quero (mandar|enviar|botar|colocar|iniciar|comecar|que (faca|imprima|agende|inicie)))\b",
    re.IGNORECASE,
)
...
def responder(pergunta: str, resultado_router: dict) -> str:
    ...
    if resultado_router.get("livre"):
        # Catalogo local (data/conhecimento.json) primeiro: se a pergunta e
        # conhecida, responde sem gastar chamada de API (e continua
        # funcionando mesmo sem chave configurada ou com a API fora).
        do_catalogo = conhecimento.buscar(pergunta)
        if do_catalogo:
            return do_catalogo
...
def processar(texto: str, texto_falado: str | None = None,
              operador: str | None = None, permitir_acoes: bool = True) -> dict:
    """
    ...
    Parametros:
        ...
        operador     — opcional: quem falou (biometria de voz). Usado para
                       auditar quem PEDIU e quem AUTORIZOU cada acao no catos.
    ...
    try:
        ...
        # --- Canal SEM permissao de acao (ex.: Alexa): recusa pedidos de acao
        # de forma clara e nao mexe no estado de confirmacao pendente (que pode
        # ter sido aberto pelo painel/voz local por um operador identificado).
        if not permitir_acoes and _eh_pedido_de_acao(texto):
            intencao = "acao_bloqueada_canal"
            resposta = ("Por seguranca, acoes de producao nao estao liberadas por "
                        "este canal. Use o painel do JARVIS para iniciar, agendar "
                        "ou registrar producao.")
        ...
        # So vale para canais com permissao de acao (nunca pela Alexa, para nao
        # confirmar por engano uma acao aberta por outra pessoa no painel).
        if permitir_acoes and agente_acoes.tem_confirmacao_pendente():
            if agente_acoes.eh_confirmacao(texto):
                ...
                resultado = {"intencao": intencao, "ok": True, "dados": None,
                             "resumo": resposta, "insuficiente": False}
                return _finalizar(falado, texto, intencao, "catos.acao",
            ...
            resultado = {"intencao": "acao_aguardando", "ok": True, "dados": None,
                         "resumo": resposta, "insuficiente": False}
            return _finalizar(falado, texto, "acao_aguardando", "catos.acao",
                              resultado, resposta, None, False, modo, entidades)
        ...
        if permitir_acoes and acoes_ok and _eh_pedido_de_acao(texto):
        ...
        # --- Caminho MCP (DADOS REAIS): se o catos estiver conectado, o agente
