...
core/mcp_client.py
------------------
Cliente MCP da JARVIS — conexao com o sistema real da empresa (catos).

O catos e um servidor MCP (Model Context Protocol) que da acesso aos dados
...
A JARVIS conecta como CLIENTE MCP e usa as ferramentas do catos para responder
...
REGRAS (inviolaveis):
    ...
    2. O TOKEN vive so no .env (CATOS_MCP_TOKEN), nunca no codigo. Mesma regra do
       KILO_API_KEY. Ver [[llm-provedor-kilo-sem-free]].

Configuracao (.env):
    CATOS_MCP_URL      = http://192.168.148.19:18080/mcp/
    CATOS_MCP_TOKEN    = <token fornecido pelo Gabriel>
    CATOS_MCP_ENABLED  = auto | 1 | 0   (auto = liga se URL+token validos existirem)
...
def _config() -> dict:
    return {
        "url": os.getenv("CATOS_MCP_URL", "").strip(),
        "token": os.getenv("CATOS_MCP_TOKEN", "").strip(),
        "enabled": os.getenv("CATOS_MCP_ENABLED", "auto").strip().lower(),
    }
...
def disponivel() -> tuple[bool, str]:
    """
    Diz se da pra usar o MCP do catos agora.
    Retorna (True, "") ou (False, motivo legivel).
    """
    cfg = _config()

    if cfg["enabled"] in ("0", "false", "nao", "off", "desligado"):
        return False, "MCP desativado no .env (CATOS_MCP_ENABLED=0)"

    if not cfg["url"]:
        return False, "CATOS_MCP_URL nao configurada no .env"

    if _token_placeholder(cfg["token"]):
        return False, ("CATOS_MCP_TOKEN ausente/placeholder — peca o token ao Gabriel "
                       "e coloque no .env")
...
def _abrir_sessao():
    ...
    inicializada e conectada ao catos via Streamable HTTP.
...
def eh_somente_leitura(tool) -> bool:
    ...
    # O VERBO-PREFIXO (primeiro pedaco do nome) decide primeiro. O catos usa a
...
def listar_ferramentas() -> list[dict]:
    """
    Conecta ao catos e devolve a lista de ferramentas (todas), cada uma como dict
...
# Cache das ferramentas de leitura. Os schemas do catos nao mudam durante uma
...
def chamar_ferramenta(nome: str, argumentos: dict | None = None) -> str:
    """
    Chama uma ferramenta de LEITURA do catos e devolve o resultado como texto.
