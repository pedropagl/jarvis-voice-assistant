...
--------------------
Agente de ACOES da JARVIS sobre o catos (escrita no sistema real).
...
def _normalizar_demandas(resultado_bruto: str) -> str:
    ...
    try:
        ...
        # Reconstroi como objetos concatenados (formato original do catos)
        ...
        for tc in tool_calls:
            ...
            if nome in nomes_acao_validos:
                ...
                try:
                    ...
                    # Isso evita o catos retornar vazio quando o operador digita
...
def executar_confirmado(operador: str | None = None) -> str:
    ...
    try:
        for i, passo in enumerate(passos):
            ...
            # (agendamento) nao gerou schedule_uuid — entao NAO envie nada ao catos
...
def _checar_aviso_material(ferramenta: str, resultado_bruto: str) -> str | None:
    """
    Se a ferramenta for enviar_para_impressora e o catos retornou aviso de
    ...
    if not esperado_label and avisos:
        ...
        esperado_label = None  # o catos nao manda o label esperado, so o guid
...
def disponivel() -> tuple[bool, str]:
    """Requer catos conectado + provedor Kilo."""
