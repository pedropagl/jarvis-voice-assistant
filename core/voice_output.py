...
def _sintetizar_edge(texto: str, destino: str = _ARQUIVO_TMP) -> str | None:
    """Gera um mp3 da fala com edge-tts via subprocess (evita bugs asyncio Win32).
    Devolve o caminho do mp3 (destino) ou None se falhar."""
    import subprocess
    try:
...
def sintetizar_mp3_bytes(texto: str) -> bytes | None:
    """
    Gera o MP3 da fala (voz neural edge-tts) e devolve os BYTES do arquivo,
    SEM tocar nada no PC. Usado pelo painel para entregar o audio ao navegador
    (celular/PC de quem esta acessando). Cada chamada usa um arquivo temporario
    proprio (evita colisao entre requisicoes simultaneas) e o apaga no fim.

    Retorna None se o texto for vazio ou se o edge-tts nao estiver disponivel.
    """
    texto = (texto or "").strip()
    if not texto:
        return None
    fd, caminho = tempfile.mkstemp(suffix=".mp3", prefix="jarvis_web_")
    os.close(fd)
    try:
        if not _sintetizar_edge(texto, caminho):
            return None
        with open(caminho, "rb") as arq:
            return arq.read()
    except Exception as erro:  # noqa: BLE001
        log.warning("Falha ao gerar mp3 para o navegador (%s).", erro)
        return None
    finally:
        try:
            os.remove(caminho)
