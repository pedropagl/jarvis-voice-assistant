...
Uso:
    ...
    python main.py mcp             -> conecta ao catos e LISTA as ferramentas (sem acoes).

FASE 14 - Dados reais via MCP (catos): quando CATOS_MCP_URL/TOKEN estao no .env,
...
# auditar quem autoriza cada acao no catos (quem mandou imprimir/agendar).
...
def boot() -> dict:
    ...
    if mcp_ok:
        ...
        # rapida e ja validamos a conexao com o catos no boot.
        try:
            n = len(mcp_client.ferramentas_de_leitura())
            mcp_status = f"catos CONECTADO ({n} ferramentas de leitura)"
        except Exception as erro:  # noqa: BLE001
            mcp_status = f"catos indisponivel agora ({erro}); usando MOCK"
    else:
...
def diagnostico_mcp() -> None:
    """
    Conecta ao MCP do catos e LISTA as ferramentas disponiveis (somente leitura
    marcadas). NAO executa nenhuma acao — so introspecciona o que o sistema oferece.
    Use isto para descobrir o que o catos e capaz de fazer.
    """
    print("\n========== MCP catos - FERRAMENTAS DISPONIVEIS ==========\n")

    ok, motivo = mcp_client.disponivel()
    if not ok:
        print(f"Nao da pra conectar ao catos agora: {motivo}\n")
        print("Configure no .env: CATOS_MCP_URL e CATOS_MCP_TOKEN (token com o Gabriel).")
        print("E instale a lib do MCP:  pip install mcp")
        print("\n=========================================================\n")
        return

    print("Conectando ao catos...")
    try:
...
if __name__ == "__main__":
    ...
    if diag_mcp:
        # Conecta ao catos e lista as ferramentas (introspeccao, sem acoes).
        diagnostico_mcp()
    elif cadastrar:
